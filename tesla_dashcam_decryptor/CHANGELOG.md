...
## 0.7.47
- **The interface is English throughout.** The last German string, the Keys panel heading "Direkt-API (automatisch)", is now "Direct API (automatic)". The `trips_subpath` default `Fahrten` is unchanged because it names the folder an existing companion recorder writes to
